...
public class WitcherRecipeProvider extends FabricRecipeProvider {
    // Fabric API 0.92: `FabricRecipeProvider(FabricDataOutput)` and `generate(Consumer<RecipeJsonProvider>)`.
    // The 2-arg constructor is kept so `pack.addProvider(WitcherRecipeProvider::new)` still resolves as a
    // RegistryDependentFactory.
    ...
    private void generateSwordRecipes(Consumer<RecipeJsonProvider> exporter) {
        // Iron Witcher Sword
        ...
        var hardenedLeather = getOrFallback(new Identifier("more_rpg_classes", "hardened_leather"), Items.LEATHER);
    ...
    private void generateModdedSwordRecipes(Consumer<RecipeJsonProvider> exporter) {
    ...
    private void createSwordRecipe(Consumer<RecipeJsonProvider> exporter, Item result, Item material) {
        var hardenedLeather = getOrFallback(new Identifier("more_rpg_classes", "hardened_leather"), Items.LEATHER);
    ...
    private void generateSpellBookRecipes(Consumer<RecipeJsonProvider> exporter) {
        var hardenedLeather = getOrFallback(new Identifier("more_rpg_classes", "hardened_leather"), Items.LEATHER);;
    ...
    private void createArmorSet(Consumer<RecipeJsonProvider> exporter, String name, Item primary, Item secondary, Armor.Set armorSet) {
        // Helmet
    ...
    private void generateDiagramRecipes(Consumer<RecipeJsonProvider> exporter) {
        var enhancedDiagram = getOrFallback(new Identifier(MOD_ID, "enhanced_diagram"), Items.LEATHER);
        var superiorDiagram = getOrFallback(new Identifier(MOD_ID, "superior_diagram"), Items.LEATHER);
        var mastercraftedDiagram = getOrFallback(new Identifier(MOD_ID, "mastercrafted_diagram"), Items.LEATHER);
        var grandmasterDiagram = getOrFallback(new Identifier(MOD_ID, "grandmaster_diagram"), Items.LEATHER);
    ...
    private void createDiagramRecipes(Consumer<RecipeJsonProvider> exporter, String name, Item diagram) {
        var hardenedLeather = getOrFallback(
                new Identifier("more_rpg_classes", "hardened_leather"),
                Items.LEATHER
        );
        ...
        ShapedRecipeJsonBuilder.create(RecipeCategory.COMBAT, diagram, 2)
                ...
                .offerTo(exporter, new Identifier(MOD_ID, name + "_diagram"));
    }
    ...
    private void generateTrinketRecipes(Consumer<RecipeJsonProvider> exporter) {
        // School Medallions
    ...
    private void createGlyphRecipe(Consumer<RecipeJsonProvider> exporter, Item result, Item ingredient) {
    ...
    private void generateDisassemblyRecipes(Consumer<RecipeJsonProvider> exporter) {
        // Armor Disassembly
    ...
    private void generateUncraftingRecipes(Consumer<RecipeJsonProvider> exporter) {
