...
public class WitcherTrinketItem extends TrinketItem {
    ...
    /// 1.20.1 Trinkets 3.7.2: the hook is keyed by a slot-unique `UUID` and speaks in raw `EntityAttribute`s
    /// (the 1.21 `Identifier` + `RegistryEntry<EntityAttribute>` shape does not exist).
    @Override
    public Multimap<EntityAttribute, EntityAttributeModifier> getModifiers(ItemStack stack, SlotReference slot, LivingEntity entity, UUID uuid) {
        ...
        for (var entry : this.customAttributes.modifiers()) {
            // Entries carry an attribute *id*: one whose attribute is not registered on this runtime
            // resolves to null and is skipped, exactly like `ItemAttributeModifiers#forSlot`.
