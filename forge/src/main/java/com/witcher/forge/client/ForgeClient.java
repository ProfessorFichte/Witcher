...
/// Forge 47 has no `@EventBusSubscriber(value = Dist.CLIENT)` with NeoForge's semantics, so the client
/// listeners are registered explicitly and only from behind a `FMLEnvironment.dist == Dist.CLIENT`
/// guard in `ForgeMod` - a dedicated server never classloads this.
