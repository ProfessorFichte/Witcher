...
public class GlyphTooltipRenderer implements TooltipComponent {
    private static final Identifier EMPTY_SLOT_TEXTURE = WitcherClassMod.id("textures/gui/empty_glyph_slot.png");
    private final GlyphSlots slots;

    public GlyphTooltipRenderer(GlyphTooltipComponent component) {
    ...
    public int getHeight() {
        ...
        int emptySlots = slots.maxSlots() - slots.attachedGlyphs().size();
    ...
    private int countUniqueItems() {
        ...
        for (ItemStack glyph : slots.attachedGlyphs()) {
            String key = glyph.getItem().toString();
    ...
    public void drawItems(TextRenderer textRenderer, int x, int y, DrawContext context) {
        ...
        Text text = Text.translatable("item.witcher_rpg.glyph_slots.tooltip")
            .append(String.format("%d/%d", slots.attachedGlyphs().size(), slots.maxSlots()));
        context.drawText(textRenderer, text, x, y, 0xAAAAAA, false);

        // Show hint or details based on Alt key
        if (!Screen.hasAltDown()) {
            // Show hint in grey
            Text hint = Text.translatable("item.witcher_rpg.glyph_render.tooltip");
            context.drawText(textRenderer, hint, x, y + 10, 0x808080, false);
        } else {
            // Show glyph items and empty slots
            int slotY = y + 12;
            int slotX = x;

            for (ItemStack glyph : slots.attachedGlyphs()) {
                context.drawItem(glyph, slotX, slotY);
                slotX += 18;
            }

            for (int i = slots.attachedGlyphs().size(); i < slots.maxSlots(); i++) {
                context.drawTexture(
                    EMPTY_SLOT_TEXTURE,
                    slotX, slotY,
                    0, 0,
            ...
            for (ItemStack glyph : slots.attachedGlyphs()) {
                String key = glyph.getItem().toString();
                String displayName = glyph.getName().getString();
                itemCounts.put(key, itemCounts.getOrDefault(key, 0) + 1);
            ...
            if (emptySlots > 0) {
                Text emptyText = Text.literal("- " + emptySlots + "x ")
                    .append(Text.translatable("item.witcher_rpg.empty_glyph_slot"));
