...
public class WitcherStatusEffects {
    ...
    public static Effects.Entry IGNI_ROLL = add(new Effects.Entry(new Identifier(MOD_ID,"igni_roll"),
            "Igni Roll",
    ...
    public static void tickIgniRollVisuals(LivingEntity entity) {
        if (entity.getWorld().isClient() || !entity.hasStatusEffect(IGNI_ROLL.effect)) {
            return;
        }
        if (entity.age % 3 == 0) {
    ...
    public static Effects.Entry MONSTER_EXPERT = add(new Effects.Entry(new Identifier(MOD_ID,"monster_expert"),
            "Monster Expert",
    ...
    private static boolean hasSpell(PlayerEntity player, String path) {
        var entry = SpellLookup.entry(player.getWorld(), new Identifier(MOD_ID, path));
        if (entry == null) {
            return false;
        }
    ...
    private static void triggerQuenExplosiveShieldBurst(PlayerEntity wearer) {
        ...
        var helperEntry = SpellLookup.entry(world, new Identifier(MOD_ID, "helpers/quen_explosive_shield_burst"));
        if (helperEntry == null) {
            return;
        }
    ...
    // Split out of register() so Forge can run it before its own registration loop.
    public static void configure() {
        ...
        CombatEvents.ENTITY_DAMAGE_TAKEN.register((args) -> {
            if (!args.entity().getWorld().isClient()
                    ...
                    && args.entity().hasStatusEffect(WITCHER_SENSES_EXPOSED.effect)
                    && hasSpell(attacker, "spell_modifiers/witcher_senses_monster_expert")) {
                CustomMethods.applyStatusEffect(args.entity(), 0, 6, MONSTER_EXPERT.effect, 4, true, true, false, 0);
            }
        });
