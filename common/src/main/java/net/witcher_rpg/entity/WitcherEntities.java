...
public class WitcherEntities {
    ...
    /// Creation only, and idempotent: builds both entity types, fills the static fields (Forge's
    /// `RegisterHelper#register` returns void, so nothing hands the type back the way
    /// `Registry.register` did) and returns the map keyed by registration id.
